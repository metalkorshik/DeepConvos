...
LanguageMenu = () => {
     ...
     let LanguageBlock = document.querySelector('.language div');
     let k = 0;
...

{
     ...
     let btnsOpenResisionForm = document.querySelectorAll('.btn-revision-sketch');
     let btnsOpenCancelForm = document.querySelectorAll('.btn-cancel-sketch');
     let btnOpenSuccessForm = document.getElementById('btn-success-message-sketch');
     let btnOpenCommentCancelForm = document.getElementById('btn-comment-cancel-sketch');
     let btnCancelMeeting = document.querySelector('.btn--cancel--meeting');
     let cancelSketchReasons = document.querySelectorAll('.radio-cancel-sketch-reasons');
     
     let closeForm = document.querySelectorAll('.close-form i');

     for (let index = 0; index < btnsOpenResisionForm.length; index++) {

          btnsOpenResisionForm[index]?.addEventListener('click', function (ev) {
               modal().open();
               form( '.sketch-form').open();
               document.getElementById('revision-sketch-id').value = this.dataset.id;
          });

     }

     for (let index = 0; index < btnsOpenCancelForm.length; index++) {

          btnsOpenCancelForm[index]?.addEventListener('click', function (ev) {
               modal().open();
               form( '.sketch-canceling-form').open();
               document.getElementById('cancel-form-sketch-id').value = this.dataset.id;
          });

     }

     btnOpenSuccessForm?.addEventListener('click', function (ev) {
          form( '.sketch-cause').close();
          modal().close();
          ...
          setTimeout(e => {
               ...
               Location.replace('/sketches');
               let comment = document.getElementById('cancel-sketch-comment').value;
               let reason = '';

               for (let index = 0; index < cancelSketchReasons.length; index++) {
                    
                    item = cancelSketchReasons[index];

                    if(item.checked)
                         reason = item.dataset.id;

               }

               server.request('/send-cancel-sketch-review', { 'order_id' : this.dataset.order, 'reason_id' : reason, 'comment' : comment })
               .then(data => {
     ...
     document.getElementById('close-submit-form').addEventListener('click', function (ev) {
          location = '/sketches';
     });

     if(btnOpenCommentCancelForm.dataset.canceled)
     {
          modal().open();
          form( '.sketch-cause').open();
     }
   
     btnCancelMeeting?.addEventListener('click', function (ev) {
          modal().open();
          form( '.meeting-canceling-form').open();
     });

     document.getElementById('deny-sketch-cancelling-btn').addEventListener('click', function (ev) {
          modal().close();
...

{
     ...
     let sketchDownloadBtns = document.querySelectorAll('.download-sketch-btn');

     for (let index = 0; index < sketchDownloadBtns.length; index++) {

          sketchDownloadBtns[index].addEventListener('click', function () {
             
               let form = document.getElementById('sketch-form-' + this.dataset.id);
     ...
     let acceptSketchBtns = document.querySelectorAll('.accept-sketch-btn');

     for (let index = 0; index < acceptSketchBtns.length; index++) {

          acceptSketchBtns[index].addEventListener('click', function (e) {
